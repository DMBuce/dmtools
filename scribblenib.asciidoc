...
monsters never need a monster-specific "target number" or "difficulty class" to roll against.
For the same reason, they also don't need numbers assigned to their stats.

A monster's stat block is just its name, number of stats, Grit,
and any abilities it has.
A bear's stat block is simply "Bear (3 stats)".
Here are a few more example stat blocks.

**Vampire** (3 stats)::
Deals damage to STR before other stats.
Gains Advantage to STR for 1 round when dealing damage with its bite.

**Troll** (3 stats, 1 Grit)::
Regrows lost limbs and heals one stat at the end of its turn.

//**Hydra** (3 stats, 1 Grit)::
